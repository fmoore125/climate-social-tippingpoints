...
baselines=c(0,1)
nethom_sweep=c(0.35,0.95)

params=expand.grid(biassedass_sweep,perception_sweep,baselines,nethom_sweep)
...
for(i in 1:dim(params)[1]){
  ...
  if(params[i,3]==0){
    m=model(biassedassimilation=params[i,1],evidenceeffect=params[i,2],shiftingbaselines=params[i,3],homophily_param=params[i,4])
...
colnames(params)=c("biassed_assimilation","evidenceeffect","shiftingbaseline","networkhomophily","opposers2050")
...
params$networkhomophily=as.factor(params$networkhomophily);params$networkhomophily=fct_recode(params$networkhomophily,"Mixed Population"="0.35","Unmixed Population"="0.95")
