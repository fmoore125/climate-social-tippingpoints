...
emis=emis%>%
  filter(REGION=="World")%>%
  filter(VARIABLE%in%c("CMIP6 Emissions|CH4","CMIP6 Emissions|CO2","CMIP6 Emissions|NH3"))%>%
...
emis=emis%>%
  filter(VARIABLE%in%c("CMIP6 Emissions|CH4","CMIP6 Emissions|NH3"))
...
a=a+annotate("text",label="y=0.04+0.46x",x=0.3,y=0.7)+labs(x="Fraction CO2 Reduced (from RCP7)",y="Fraction Other Gas Reduced (from RCP7)")
