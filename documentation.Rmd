...
Adoption->{Emissions Opinion} Policy->Emissions Emissions->Climate
...
<p style="font-size:14pt">Population is divided into three types based on opinion ($Op$) regarding climate policy - Supporting, Neutral, and Opposed (i.e. $\mathbf{Op} = \left[S,N,O\right]$). Movement between adjacent types is determined by up to three things: a social pressure or persuasive force based on interactions between groups given the group network structure ($Force$), evidence derived from one's own experience of weather ($Ev$), and a feedback from policy to opinion driven by individuals updating their information about the social norm based on institutiona (i.e. policy) change ($PolOp$).</p>
...
<p style="font-size:14pt">In addition to social persuasion, transition between opinion groups is also determined directly by the experience of climate chance through the perception of weather anomalies. This evidence effect ($Ev$) is allowed to differ by opinion group (reflecting biassed assimilation or other forms of motivated reasoning), can take positive or negative values (depending on whether the perceived weather anomaly is warm or cold), and is scaled by the "evidence effectiveness" parameter, ($\eta$) which determines how strongly perceived weather anomalies affect opinion. More details on how the evidence based on climate change is calculated is in the Cognition Component.</p>

<p style="font-size:14pt">Finally, we also allow for two possible feedbacks either changes in policy or adoption of individual behaviors can influence opinion. Evidence in the literature for both of these effects is fairly weak (e.g. Tankard and Paluck, 2016), but these both constitute potentially important positive feedback effects.</p>

<p style="font-size:14pt">The hypothetical feedback from policy to opinion operates by providing evidence on the population-wide norm (as oppposed to the norm experienced by individuals which depends on the social network). The strenght of this effect depends on the change in policy last period ($\Delta Policy_{t-1}$) and a scaling parameter $\kappa$:
...
<p style="font-size:14pt">The hypothetical feedback from adoption to opinion is similarly governed by a single parameter, $\xi$, which gives the effect in percentage points of increasing support for climate policy after adopting sustainable behaviors. Note that, unlike the policy opinion feedback, this effect is unidirectional (it can only take positive values) - adopting more sustainable behaviors can only increase support for climate policy.</p>
...
{rrr}
(1-(\sum_{j=2,3}SocialForce_{1,j}-\eta Ev_{1,t-1}-PolOp_{t}-\xi A_{1,t-1}) & \sum_{j=2,3}SocialForce_{1,j}-\eta Ev_{1,t-1}-PolOp_{t}-\xi A_{1,t-1} & 0 \\
SocialForce_{2,1}+\eta Ev_{2,t-1}+PolOp_{t}+\xi A_{2,t-1} & 1-(\sum_{j=1,3}SocialForce_{2,j}) & SocialForce_{2,1}-\eta Ev_{2,t-1}-PolOp-\xi A_{2,t-1} \\
0 &  \sum_{j=1,2}SocialForce_{3,j}+\eta Ev_{3,t-1}+PolOp_{t}+\xi A_{3,t-1} & (1-(\sum_{j=1,2}SocialForce_{3,j}+\eta Ev_{3,t-1}+PolOp_{t}+\xi A_{3,t-1})
\end{array}\right]$$</p>

<p style="font-size:14pt">Where $A_{i,t-1}$ is the fraction of adopters in opinion group $i$ in the previous time period (described in the Adoption Component)</p>
...
$$m_{t}=
\begin{cases}
\gamma_{t}*(1-\frac{log(300-Policy_{t})}{log(300)}) & \text{it } Policy_{t}<300\\
...
<p style="font-size:14pt">Note that the exponent gives the number of doublings relative to the initial $\gamma$ value. This is a representation of a "single-factor learning curve" described by Rubin et al. (2015). Their review gives ranges for the $lbd$ parameter between 0 and 30%.</p>
...
<p style="font-size:14pt">The climate model is based on the DICE2016 model, annualized using formulas from Cai, Judd and Lontzek (2012). This is a three box carbon-cycle model (atmopsphere, upper ocean, and lower ocean) that tracks both atmopspheric and ocean temperature. Details are available in: [Cai, Y., Judd, K., and Lontzek, T. S., 2012. "DSICE: A Dynamic Stochastic Integrated Model of Climate and Economy", RDCEP Working Paper No 12-02](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=1992674)</p>
