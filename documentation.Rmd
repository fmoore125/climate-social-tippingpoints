...
<p style="font-size:14pt">Finally, we also allow for a feedback where changing policy can influence opinion by providing evidence on the population-wide norm (as oppposed to the norm experienced by individuals which depends on the social network). Evidence for the existence of this effect is not particularly strong (Tankard and Paluck, 2016), but it constitutes a potentially important feedback mechanism. The strenght of this effect depends on the change in policy last period ($\Delta Policy_{t-1}$) and a scaling parameter $\kappa$:
...
{rrr}
(1-(\sum_{j=2,3}SocialForce_{1,j}-\eta*Ev_{1}-PolOp) & \sum_{j=2,3}SocialForce_{1,j}-\eta*Ev_{1}-PolOp & 0 \\
SocialForce_{2,1}+\eta*Ev_{2}+PolOp & (1-(\sum_{j=1,3}SocialForce_{2,j}) & SocialForce_{2,1}-\eta*Ev_{2}-PolOp \\
0 &  \sum_{j=1,2}SocialForce_{3,j}+\eta*Ev_{3}+PolOp & (1-(\sum_{j=1,2}SocialForce_{3,j}+\eta*Ev_{3}+PolOp)
\end{array}\right]$$</p>
