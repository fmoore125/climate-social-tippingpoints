...
<p style="font-size:14pt">Finally, we also allow for a possible feedbacks allowing changes in policy to influence opinion. Evidence in the literature for this effects is fairly weak (e.g. Tankard and Paluck, 2016), but it is a potentially important positive feedback effect.</p>
...
{rrr}
(1-(\sum_{j=2,3}SocialForce_{1,j}-\eta Ev_{1,t-1}-PolOp_{t}) & \sum_{j=2,3}SocialForce_{1,j}-\eta Ev_{1,t-1}-PolOp_{t} & 0 \\
SocialForce_{2,1}+\eta Ev_{2,t-1}+PolOp_{t} & 1-(\sum_{j=1,3}SocialForce_{2,j}) & SocialForce_{2,1}-\eta Ev_{2,t-1}-PolOp \\
0 &  \sum_{j=1,2}SocialForce_{3,j}+\eta Ev_{3,t-1}+PolOp_{t} & (1-(\sum_{j=1,2}SocialForce_{3,j}+\eta Ev_{3,t-1}+PolOp_{t})
\end{array}\right]$$</p>
